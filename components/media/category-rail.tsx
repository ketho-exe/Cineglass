  ...
  return (
    <section className="glass mt-8 rounded-3xl p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.22em] text-cyan-200">Browse by category</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {genres.map((genre) => (
          <Link key={genre.id} href={`/genre/${mediaType}/${genre.id}`} className="rounded-full border border-white/10 bg-white/[0.07] px-4 py-2 text-sm text-slate-100 backdrop-blur transition hover:border-cine-accent/50 hover:bg-cine-accent/10">
            {genre.name}
          </Link>
        ))}
      </div>
